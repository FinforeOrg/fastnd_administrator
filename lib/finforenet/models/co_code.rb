  ...
  module Models
    module CoCode
      def self.company_twitter(path="")
       ...
       def self.async_populate_rss
         header = {:id => 0, :title => 1, :category => 2, :address => 3, :industry => 4, :geography => 5, :profession => 6, :autopopulate => 7}
         path = "#{Rails.root}/autopopulate_rss.csv" if path.blank?
         csv_file = File.new(path).read
         csv_data = CSV.parse(csv_file)
